...
def print_mislabeled_images(X, y, p):
    ...
    plt.rcParams['figure.figsize'] = (128.0, 128.0) # set default size of plots
    num_images = len(mislabeled_indices[0])
    for i in range(num_images):
        index = mislabeled_indices[1][i]
        
        plt.subplot((int(num_images/6) + 1) * 2, 6, i + 1)
        plt.imshow(X[:,index].reshape(128,128,3), interpolation='nearest')
        plt.axis('off')
        plt.title("Prediction: " + ("Pothole" if int(p[0,index]) == 1 else "Normal") + " \n Actual: " + ("Pothole" if int(y[0,index]) == 1 else "Normal"), fontsize=40)
